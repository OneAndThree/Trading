...
public class YahooFetchRealTimeData {
    ...
    public Map<String, Object> getRealtimeData() {
        ...
        Map<String, Object> EquitiesMap = new HashMap<String, Object>();

        for (String EquityString : Equities) {
            String EquityS = EquityString.replaceAll("\"", "");
            String[] equityItem = EquityS.split(",");

            Map<String, String> equitiDetail = new HashMap<String, String>();

            for (int num = 0; num < equityItem.length; num++) {
                equitiDetail.put(tags[num], equityItem[num]);
            }
            EquitiesMap.put(equitiDetail.get("s"), equitiDetail);
